...
file :: IO String
file = readFile "inconsistent20.txt"
...
isNotBlockedUnit :: [String] -> [(String, [Int])] -> Bool
isNotBlockedUnit unit board = all (\sq -> isNonBlockedSquare sq board) unit
...
-- by checking if there are any squares for which only values which conflict with its peers are allowed to be placed.
-- we also check that there are no single-element duplicate lists, i.e. direct conflicts.
validUnit :: [String] -> [(String, [Int])] -> Bool
validUnit unit board = containsNoSingleDuplicates unit board && isNotBlockedUnit unit board
